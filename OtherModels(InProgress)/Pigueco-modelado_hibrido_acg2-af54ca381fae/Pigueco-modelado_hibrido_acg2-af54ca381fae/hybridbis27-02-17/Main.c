...
int main(int argc, char **argv)
{
    ...
    //there is something like that in the header file. However, this variable here
    //is for the purpose of directly asking the user to provide this during running time
    ...
    int time_counter=1;
    int file_counter=1;//MAY cause trouble if we want a large number of output files
    //(long-to-string conversion by means of itoa has not been tested...)
    ...
    for(i=0;i<Roberto_lines;i++){
        fscanf(ROBERTO_FILE,"%lf %lf",RobertoNcells+i, RobertoAges+i);
    };
   ...
   //Se inicializa en el bucle del Gillespie
    ...
    //Think about this, we may not be gaining much computation time after all.
    ...
    while(t<tstop){
        ...
        r1=ran2(&seed);//drand48(); //This was Roberto's choice
		tau=log(1/r1)/suma;
       ...
       //PRINTING DATA ON SELECTED ITERATIONS
        //again the issue numbers vs densities
       	...
       	if(t+tau>=time_counter*sampling_time_window){ //print the last state prior surpassing the time mark

       		time_counter++;
            while(t+tau>=time_counter*sampling_time_window){time_counter++;};
            ...
            strcpy(output_path1,label1);
            Print_Vector(OUTPUT_DATA,output_path1,time_counter-1,n_xslots,density_of_cells);
            //REPLACE time_counter-1 by file_counter if we want output files to be labelled
            //in a correlative way (however if we find less than N_OUTPUTS output files then the labelling does not correspond to the actual times and those have to be traced
            //from the time points at the population file)
            
            //On the other hand, with the current labelling we may find some trouble when runnig scripts on the set of output files as there may be some "holes" (corresponding to those rare instances in which tau is extremely large)
            
            //Printing oxygen spatial distribution
            strcpy(output_path2,label2);
            Print_Vector(OUTPUT_DATA,output_path2,time_counter-1,n_xslots,oxygen_level);
            //Same as before...

            printf("t=%lf\n",(time_counter-1)*sampling_time_window);
        ...
        //and when we sweep we DO include the interface
