...
void Initialize_propensities(struct sim_parameters *params,
  ...
  Compute_division_threshold(p,
    ...
    ref_index=0;//no cell committed to divide
    ...
    (StochasticAges[i]->propensity_vector)[DEATH]=(StochasticAges[i]->ncells)*death_rate;
    ...
    *pfiring_reaction=j; //reaction tag
    ...
    switch(j){ //doing things like case '0': causes it to fail
      ...
      case BIRTH: //birth 0
        ...
        if(k>=(StochasticAges[i]->ncells_ready_to_divide)){ //hopefully not needed
...
void Handle_event(struct sim_parameters *params,
    ...
    switch(firing_reaction){ 
      ...
      (StochasticAges[firing_slot-1]->propensity_vector)[DEATH]+=death_rate;
