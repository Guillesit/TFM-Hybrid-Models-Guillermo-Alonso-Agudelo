...
double Compute_total_propensity(struct age_structure **StochasticAges,
                                long n_xslots, long interface){

    long i=interface;
    double total_propensity=0.0;

    for (i = interface; i < n_xslots; i++){
...
void Determine_event(struct all_parameters *params,
    ...
    long i=interface; //spatial location
    ...
    //printf("%lf\n",(StochasticAges[interface]->propensity_vector)[0]);
    sum=(StochasticAges[interface]->propensity_vector)[0];

    //printf("%lf\n",fraction_of_total_propensity);
    while(sum<fraction_of_total_propensity){
        ...
        //printf("%ld %d %lf\n",i,j,(StochasticAges[i]->propensity_vector)[j]);
    ...
    switch(j){ //doing things like case '0': causes it to fail
      ...
      case BIRTH: //birth 0
        ...
        if(k>=(StochasticAges[i]->ncells)){ //hopefully not needed
          fprintf(stderr,"Something went wrong when determining firing_cell (birth),%ld\n",i);
          exit(1);
        };
        *pfiring_cell=k;
...
void Update_interface(struct all_parameters *parameters,
                        struct age_structure **StochasticAges,
                        double *ncells_per_slot,
                        long interface){
        //0=birth, 1=death, 2=left diffusion, 3=right diffusion
        (StochasticAges[interface]->propensity_vector)[BIRTH]=ncells_per_slot[interface]*parameters->r;//tau_p;
        if (ncells_per_slot[interface]>0){
        (StochasticAges[interface]->propensity_vector)[DEATH]=ncells_per_slot[interface]*(ncells_per_slot[interface]-1)*parameters->r/parameters->K;
        } else {
          (StochasticAges[interface]->propensity_vector)[DEATH]=0.0;
        }
         //cannot diffuse to the left at the left end
            (StochasticAges[interface]->propensity_vector)[LDIFF]=0.0;
        
        if(interface==parameters->n_xslots-1){ //cannot diffuse to the right at the right end
            (StochasticAges[interface]->propensity_vector)[RDIFF]=0.0;
        }
        else{
            (StochasticAges[interface]->propensity_vector)[RDIFF]=ncells_per_slot[interface]*parameters->D/((parameters->delta_x)*(parameters->delta_x));
        };
        (StochasticAges[interface]->ncells)=ncells_per_slot[interface];
