...
{   
    ...
    */
    ...
    c=0;
    double threshold=parameters.K*0.5;
    Update_interface(&parameters,StochasticAges,ncells_per_slot,interface);

    while((ncells_per_slot[interface]>=threshold)&&(interface<=n_xslots-1)){
      interface++;
      
    }
    printf("Interface in %ld\n",interface);
    double z;
    if ((ncells_per_slot[interface]-ncells_per_slot[interface-1])==0){
      z=interface;
    } else { //Interpolate the exact point where ncells=n0
      z=(threshold-ncells_per_slot[interface-1])/(ncells_per_slot[interface]-ncells_per_slot[interface-1])+interface-1; 
      printf("z: %lf\n",z);
    }
  ...
  while (t<tstop)
  {
    ...
    if((interface<=n_xslots-1)&&(prepare_next_reaction)){
      if (std::ceil(z)>interface){printf("New interface in %ld\n",interface+1);
      }
      interface=std::ceil(z);
    }
    ...
    if(interface<=n_xslots-2){
      z+=(ncells_per_slot[interface]-ncells_per_slot[interface+1])*time_advance/threshold;
      //printf("z: %lf\n",z);
      //printf("interface: %ld\n",interface);
